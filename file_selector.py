...
class FileSelector:
    ...
    def _create_complete_analysis_tab(self):
        ...
        self.complete_columns = [
            ...
            # Plateau Analysis
            "Chg 1st Plateau (mAh/g)", "Chg 2nd Plateau (mAh/g)",
            "Chg Total (mAh/g)", "Chg Transition (V)",
            "Dchg 1st Plateau (mAh/g)", "Dchg 2nd Plateau (mAh/g)",
        ...
        column_widths = {
            ...
            "Chg 1st Plateau (mAh/g)": 120, "Chg 2nd Plateau (mAh/g)": 120,
            "Chg Total (mAh/g)": 100, "Chg Transition (V)": 100,
            "Dchg 1st Plateau (mAh/g)": 120, "Dchg 2nd Plateau (mAh/g)": 120,
    ...
    def _consolidate_all_metrics(self, features_df, dqdv_stats):
        ...
        if dqdv_stats:
            for stat in dqdv_stats:
                key = (stat.get('File', ''), stat.get('Cycle', ''))
                dqdv_dict[key] = stat
        ...
        for _, row in features_df.iterrows():
            ...
            dqdv_data = dqdv_dict.get((cell_id, cycle), {})
            ...
            consolidated_row = {
                ...
                # Plateau analysis (1 decimal for capacities, 3 for voltages)
                "Chg 1st Plateau (mAh/g)": f"{float(dqdv_data.get('Charge 1st Plateau (mAh/g)', 0)):.1f}" if dqdv_data.get(
                    'Charge 1st Plateau (mAh/g)') is not None else "N/A",
                "Chg 2nd Plateau (mAh/g)": f"{float(dqdv_data.get('Charge 2nd Plateau (mAh/g)', 0)):.1f}" if dqdv_data.get(
                    'Charge 2nd Plateau (mAh/g)') is not None else "N/A",
                "Chg Total (mAh/g)": f"{float(dqdv_data.get('Charge Total (mAh/g)', 0)):.1f}" if dqdv_data.get(
                ...
                "Dchg 1st Plateau (mAh/g)": f"{float(dqdv_data.get('Discharge 1st Plateau (mAh/g)', 0)):.1f}" if dqdv_data.get(
                    'Discharge 1st Plateau (mAh/g)') is not None else "N/A",
                "Dchg 2nd Plateau (mAh/g)": f"{float(dqdv_data.get('Discharge 2nd Plateau (mAh/g)', 0)):.1f}" if dqdv_data.get(
                    'Discharge 2nd Plateau (mAh/g)') is not None else "N/A",
                "Dchg Total (mAh/g)": f"{float(dqdv_data.get('Discharge Total (mAh/g)', 0)):.1f}" if dqdv_data.get(
